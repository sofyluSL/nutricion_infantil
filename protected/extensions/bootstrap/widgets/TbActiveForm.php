...
class TbActiveForm extends CActiveForm
{
	...
	protected function inputsList($checkbox, $model, $attribute, $data, $htmlOptions = array())
	{
		...
		{
                        $checked = !is_array($select) && !strcmp($value, $select) || is_array($select) && ( !is_object(isset($select[0]) ? $select[0] : null ) ? in_array($value, $select) : $this->buscarObjeto($value, $select));
...
EOD;
	...
	public function inputRow($type, $model, $attribute, $data = null, $htmlOptions = array())
	{
        ...
        /**
         * Busca el id dentro del objeto
         * @author Santiago Benítez <sbenitez@tradesystem.com.ec>
         * @param type $value
         * @param type $select
         * @return boolean
         */
        protected function buscarObjeto($value, $select){
            foreach($select as $sel){
                if($sel->oldValues['id'] == $value){
                    return true;
                }
            }
            return false;
